...
if [[ -n $ruby_version ]]; then
  ruby_segment="#[fg=$ruby_colors[2]]$sep rb=$ruby_version#[default]"
else
  ruby_segment=
fi
...
if [[ -n $node_version ]]; then
  node_segment="#[fg=$node_colors[2]]$sep js=$node_version#[default]"
else
  node_segment=
fi
...
if [[ -n $python_version ]]; then
  python_segment="#[fg=$python_colors[1]]$sep py=$python_version#[default]"
else
  python_segment=
fi
...
if [[ -n $node_segment && $window_width -gt $node_lower_bound ]]; then
  tmux_status_right="$tmux_status_right$node_segment"
fi
if [[ -n $ruby_segment && $window_width -gt $ruby_lower_bound ]]; then
  tmux_status_right="$tmux_status_right$ruby_segment"
fi
if [[ -n $python_segment && $window_width -gt $python_lower_bound ]]; then
  tmux_status_right="$tmux_status_right$python_segment"
fi
if [[ -n $php_segment && $window_width -gt $php_lower_bound ]]; then
